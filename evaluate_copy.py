...
Usage:
    python evaluate_copy.py --student_id <your_id> --prediction <rankings.csv>
    python evaluate_copy.py --student_id <your_id> --prediction <rankings.csv> --datasets dataset_a dataset_b
    python evaluate_copy.py --student_id <your_id> --prediction <rankings.csv> --parquet our_test.parquet
"""
...
def load_dataset_a_gt(root: str, parquet_file: str = "test.parquet"):
    """Load dataset_a ground truth with dynamic query/gallery split."""
...
def evaluate_dataset(
    ...
    if dataset_name == "dataset_a":
        query_pids, query_camids, gallery_pids, gallery_camids, n_q, n_g = load_dataset_a_gt(
            dataset_root, parquet_file)
...
def main():
    ...
    parser.add_argument(
        "--parquet", type=str, default="test.parquet",
        help="Parquet filename inside each dataset root to use for query/gallery split (default: test.parquet)",
    )
    ...
    for dataset_name in args.datasets:
        ...
        if not os.path.exists(os.path.join(dataset_root, args.parquet)):
            print(f"  ERROR: Ground truth not found at {dataset_root}/{args.parquet}")
        ...
        try:
            results = evaluate_dataset(pred_path, dataset_root, dataset_name, args.parquet)
