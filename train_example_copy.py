...
class ReIDTrainDataset(Dataset):
    ...
    def __init__(self, root: str, parquet_file: str = "our_train.parquet", image_size=(224, 224)):
        self.root = root
        ...
        df["_full_path"] = df["image_path"].apply(lambda p: os.path.join(root, p))
        df = df[df["_full_path"].apply(os.path.exists)].reset_index(drop=True)
        print(f"  Found {len(df)} images on disk (parquet had more entries)")
...
def train(args):
    ...
    if args.resume:
        ...
        print(f"Resumed from {args.resume} (epoch {checkpoint['epoch']}, loss {best_loss:.4f})")
        print(f"Continuing from epoch {start_epoch + 1}/{args.epochs}")

    # LR Scheduler: cosine annealing with warmup
    # fast-forward last_epoch so the schedule continues correctly after resume
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer, lr_lambda, last_epoch=steps_done - 1
    )
...
def load_query_gallery(root, dataset_name, parquet_file="test.parquet"):
...
def predict(args):
    ...
    query_paths, gallery_paths = load_query_gallery(
        args.dataset_root, args.dataset_name, args.parquet)
    ...
    # Compute rankings — for dataset_a, mask each query's own gallery entry
    # (query images are appended to gallery at index n_gallery_orig + q_idx,
    # which would otherwise be a trivial self-match at cosine similarity 1.0)
    print("Computing rankings...")
    similarity = np.matmul(query_emb, gallery_emb.T)
    if args.dataset_name == "dataset_a":
        n_gallery_orig = len(gallery_paths) - len(query_paths)
        for q_idx in range(len(query_paths)):
            similarity[q_idx, n_gallery_orig + q_idx] = -np.inf
...
def main():
    ...
    parser.add_argument("--parquet", type=str, default="test.parquet", help="Parquet filename inside dataset_root to use for query/gallery split")
